#!/usr/bin/env python3
"""Inject standard Farsi white footers into non-home pages (depth=1 from V0/)."""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1] / "V0"

FA_WHITE = """    <style>#ramo-news-footer a{color:#0f172a!important}#ramo-news-footer a:hover{color:#0d9488!important}</style>
...
OLD_FOOTER_RE = re.compile(
    r"(?:\s*<!-- Footer placeholder -->\s*)?"
    r'<footer class="bg-white border-t border-gray-200 mt-12 py-8">\s*'
    r'<div class="max-w-7xl mx-auto px-4 text-center text-xs text-gray-400">\s*'
    r"[^<]*"
    r"</div>\s*</footer>",
    re.DOTALL,
)
...
def replace_old_news_footer(text: str) -> tuple[str, bool]:
    new_text, n = OLD_FOOTER_RE.subn(FA_WHITE, text, count=1)
    return new_text, n > 0
...
def main():
    ...
    for path in sorted(news_dir.glob("*.html")):
        ...
        new_t, did = replace_old_news_footer(t)
        if did:
            path.write_text(new_t, encoding="utf-8")
            print("patched", path.relative_to(ROOT))

    for rel in ("articles/news-article-template.html", "articles/scientific-article-template.html"):
        path = ROOT / rel
        t = path.read_text(encoding="utf-8")
        new_t, did = replace_old_news_footer(t)
        if did:
